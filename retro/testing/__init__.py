...
for g in games:
    overlays = 0
    if os.path.exists(
        os.path.join(retro.data.path(), *retro.data.Integrations.STABLE.paths, g)
    ):
        all_games.append(g)
        overlays += 1
    if os.path.exists(
        os.path.join(
            retro.data.path(), *retro.data.Integrations.EXPERIMENTAL_ONLY.paths, g
        )
    ):
        all_games.append(g + "-exp")
        overlays += 1
    if os.path.exists(
        os.path.join(retro.data.path(), *retro.data.Integrations.CONTRIB_ONLY.paths, g)
    ):
...
def branch_new(upstream="master", downstream=None):
    ...
    try:
        diffs = subprocess.check_output(
            ["git", "diff", "--name-only"] + branches
        ).decode("utf-8")
    except subprocess.CalledProcessError:
        return []
    check = {
        name.split("/")[0].replace("-", "_")
        for name in diffs.splitlines()
        if "-" in name
    }
    return list(check)
...
@pytest.fixture(
    params=[
        ...
        for g in os.listdir(os.path.join(os.path.dirname(__file__), "../../tests/roms"))
    ]
)
def testenv(request):
    import retro.data
    ...
    retro.data.get_romfile_path = lambda game, *args, **kwargs: [
        os.path.join(path, g) for g in os.listdir(path) if g.startswith(game)
    ][0]
