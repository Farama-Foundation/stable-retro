...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-R", "--root", metavar="ROOT", default="/", help="root directory to search"
    )
    parser.add_argument(
        "-I",
        "--install-name-tool",
        metavar="INSTALL_NAME_TOOL",
        default="install_name_tool",
        help="path to install_name_tool",
    )
    parser.add_argument(
        "-O", "--otool", metavar="OTOOL", default="otool", help="path to otool"
    )
    parser.add_argument(
        "-p",
        "--qt-plugins",
        metavar="PLUGINS",
        default="",
        help="Qt plugins to include (comma-separated)",
    )
    parser.add_argument(
        "-v",
        "--verbose",
    ...
    for executable in os.listdir(os.path.join(args.bundle, "Contents/MacOS")):
        ...
        updateMachO(
            fullPath,
            splitPath(os.path.join(args.bundle, "Contents/MacOS")),
            splitPath(args.root),
        )
    if args.qt_plugins:
        try:
