        ...
        for file in files:
            ...
            if ext in (".gb", ".sym"):
                shutil.copy(
                    ...
                    os.path.join(dest, root, fname, "test" + ext),
                )
            ...
            for suffix, model in suffixes.items():
                if fname.endswith("-" + suffix):
                    manifest = {}
                    try:
                        with open(os.path.join(dest, root, fname, "manifest.yml")) as f:
                            manifest = yaml.safe_load(f) or {}
                    except OSError:
                        pass
                    dictMerge(manifest, {"config": {"gb.model": model}})
                    with open(
                        os.path.join(dest, root, fname, "manifest.yml"), "w"
                    ) as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="directory to contain ingested tests",
    )
